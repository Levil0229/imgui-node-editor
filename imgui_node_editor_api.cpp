...
void ax::NodeEditor::PushStyleColor(StyleColor colorIndex,
                                    const ImVec4& color) {
...
void ax::NodeEditor::End() { s_Editor->End(); }
...
void ax::NodeEditor::EndPin() { s_Editor->GetNodeBuilder().EndPin(); }

void ax::NodeEditor::Group(ImVec2& size) {
  s_Editor->GetNodeBuilder().Group(size);
}

void ax::NodeEditor::EndNode() { s_Editor->GetNodeBuilder().End(); }
...
void ax::NodeEditor::EndGroupHint() { s_Editor->GetHintBuilder().End(); }
...
bool ax::NodeEditor::Link(LinkId id, PinId startPinId, PinId endPinId,
...
bool ax::NodeEditor::QueryNewNode(PinId* pinId, const ImVec4& color,
                                  float thickness) {
  using Result = ax::NodeEditor::Detail::CreateItemAction::Result;
...
bool ax::NodeEditor::QueryDeletedLink(LinkId* linkId, PinId* startId,
                                      PinId* endId) {
  auto& context = s_Editor->GetItemDeleter();
...
void ax::NodeEditor::Suspend() { s_Editor->Suspend(); }

void ax::NodeEditor::Resume() { s_Editor->Resume(); }

bool ax::NodeEditor::IsSuspended() { return s_Editor->IsSuspended(); }

bool ax::NodeEditor::IsActive() { return s_Editor->IsActive(); }
...
void ax::NodeEditor::ClearSelection() { s_Editor->ClearSelection(); }
...
bool ax::NodeEditor::BeginShortcut() { return s_Editor->GetShortcut().Begin(); }

bool ax::NodeEditor::AcceptCut() { return s_Editor->GetShortcut().AcceptCut(); }
...
void ax::NodeEditor::EndShortcut() { return s_Editor->GetShortcut().End(); }

float ax::NodeEditor::GetCurrentZoom() { return s_Editor->GetView().InvScale; }
...
ImVec2 ax::NodeEditor::GetScreenSize() { return s_Editor->GetRect().GetSize(); }
