...
#define DECLARE_KEY_TESTER(Key)                                                \
  ...
  struct KeyTester_##Key {                                                     \
    ...
    static int Get(typename std::enable_if<                                    \
                   !has_nested_##Key<ImGuiKey_>::value, T>::type*) {           \
      return -1;                                                               \
    }                                                                          \
  }
...
static void ImDrawList_AddBezierWithArrows(
  ...
  if (fill) {
    drawList->AddBezierCurve(curve.P0, curve.P1, curve.P2, curve.P3, color,
                             thickness);
...
void ed::Pin::Draw(ImDrawList* drawList, DrawFlags flags) {
  if (flags & Hovered) {
    ...
    if (!Editor->IsSelected(m_Node)) m_Node->Draw(drawList, flags);
  }
}
...
ImLine ed::Pin::GetClosestLine(const Pin* pin) const {
  return ImRect_ClosestLine(m_Pivot, pin->m_Pivot, m_Radius + m_ArrowSize,
                            pin->m_Radius + pin->m_ArrowSize);
}
    ...
    if (IsGroup(this)) {
      ...
      if (m_GroupBorderWidth > 0.0f) {
        ...
        drawList->AddRect(m_GroupBounds.Min, m_GroupBounds.Max,
                          m_GroupBorderColor, m_GroupRounding, 15,
                          m_GroupBorderWidth);
      }
    }
...
void ed::Node::DrawBorder(ImDrawList* drawList, ImU32 color, float thickness) {
  if (thickness > 0.0f) {
    drawList->AddRect(m_Bounds.Min, m_Bounds.Max, color, m_Rounding, 15,
                      thickness);
  }
}

void ed::Node::GetGroupedNodes(std::vector<Node*>& result, bool append) {
  if (!append) result.resize(0);

  if (!IsGroup(this)) return;
...
ImRect ed::Node::GetRegionBounds(NodeRegion region) const {
  if (m_Type == NodeType::Node) {
    if (region == NodeRegion::Header) return m_Bounds;
...
ed::NodeRegion ed::Node::GetRegion(const ImVec2& point) const {
  if (m_Type == NodeType::Node) {
    ...
    for (auto region : c_Regions) {
      ...
      if (bounds.Contains(point)) return region;
    }
  }
...
bool ed::Link::TestHit(const ImVec2& point, float extraThickness) const {
  if (!m_IsLive) return false;

  auto bounds = GetBounds();
  if (extraThickness > 0.0f) bounds.Expand(extraThickness);
...
bool ed::Link::TestHit(const ImRect& rect, bool allowIntersect) const {
  ...
  if (!allowIntersect || !rect.Overlaps(bounds)) return false;
...
ed::EditorContext::~EditorContext() {
  if (m_IsInitialized) SaveSettings();
...
void ed::EditorContext::Begin(const char* id, const ImVec2& size) {
  ...
  if (canvasSize.x <= 0.0f) canvasSize.x = ImMax(4.0f, availableContentSize.x);
  if (canvasSize.y <= 0.0f) canvasSize.y = ImMax(4.0f, availableContentSize.y);
  ...
  //
  m_NavigateAction.SetWindow(m_Canvas.ViewRect().Min,
                             m_Canvas.ViewRect().GetSize());

  if (m_CurrentAction && m_CurrentAction->IsDragging() &&
      m_NavigateAction.MoveOverEdge()) {
    auto& io = ImGui::GetIO();
  ...
  if (HasSelectionChanged()) ++m_SelectionId;
...
void ed::EditorContext::End() {
  ...
  // DoubleClickedNode); if (DoubleClickedPin)  LOG_TRACE(0, "DOUBLE CLICK PIN:
  // %d", DoubleClickedPin); if (DoubleClickedLink) LOG_TRACE(0, "DOUBLE CLICK
  // LINK: %d", DoubleClickedLink); if (BackgroundDoubleClicked) LOG_TRACE(0,
  // "DOUBLE CLICK BACKGROUND", DoubleClickedLink);
  ...
  for (auto node : m_Nodes)
    if (node->m_IsLive && node->IsVisible()) node->Draw(drawList);

  // Draw links
  for (auto link : m_Links)
    if (link->m_IsLive && link->IsVisible()) link->Draw(drawList);
  ...
  // Draw animations
  for (auto controller : m_AnimationControllers) controller->Draw(drawList);
  ...
  if (nullptr == m_CurrentAction) {
    ...
    if (possibleAction) ImGui::SetMouseCursor(possibleAction->GetCursor());

    if (m_CurrentAction && possibleAction) possibleAction->Reject();
  }

  if (m_CurrentAction) ImGui::SetMouseCursor(m_CurrentAction->GetCursor());
  ...
  // node drawing order.
  {
    ...
    ImDrawList_ChannelsGrow(drawList, drawList->_Splitter._Count +
                                          c_ChannelsPerNode * liveNodeCount +
                                          c_LinkChannelCount);

    int targetChannel = nodeChannelCount;

    auto copyNode = [&targetChannel, drawList](Node* node) {
      if (!node->m_IsLive) return;

      for (int i = 0; i < c_ChannelsPerNode; ++i)
        ImDrawList_SwapChannels(drawList, node->m_Channel + i,
                                targetChannel + i);
    ...
    auto groupsItEnd = std::find_if(m_Nodes.begin(), m_Nodes.end(),
                                    [](Node* node) { return !IsGroup(node); });
...
#if 1  // #FIXME
  {
    ...
    drawList->AddRectFilled(VIEW_POS, VIEW_POS + VIEW_SIZE,
                            GetColor(StyleColor_Bg));

    for (float x = fmodf(offset.x, GRID_SX); x < VIEW_SIZE.x; x += GRID_SX)
      drawList->AddLine(ImVec2(x, 0.0f) + VIEW_POS,
                        ImVec2(x, VIEW_SIZE.y) + VIEW_POS, GRID_COLOR);
    for (float y = fmodf(offset.y, GRID_SY); y < VIEW_SIZE.y; y += GRID_SY)
      drawList->AddLine(ImVec2(0.0f, y) + VIEW_POS,
                        ImVec2(VIEW_SIZE.x, y) + VIEW_POS, GRID_COLOR);
  }
#endif
  ...
  // Move user and hint channels to top
  {
    ...
    ImDrawList_SwapChannels(drawList, c_UserChannel_HintsBackground,
                            channelCount + 0);
  ...
  // ImGui::EndChild();
  // ImGui::PopStyleColor();
  if (m_IsCanvasVisible) m_Canvas.End();
  ...
  // Draw border
  {
    ...
    drawList->AddRect(m_Canvas.Rect().Min + ImVec2(1, 1),
                      m_Canvas.Rect().Max - ImVec2(1, 1),
                      ImColor(borderShadoColor));
    drawList->AddRect(m_Canvas.Rect().Min, m_Canvas.Rect().Max,
                      ImColor(borderColor));
  }
  ...
  if (!m_CurrentAction && m_IsFirstFrame && !m_Settings.m_Selection.empty()) {
    ...
    for (auto id : m_Settings.m_Selection)
      if (auto object = FindObject(id)) SelectObject(object);
  }

  if (HasSelectionChanged()) MakeDirty(SaveReasonFlags::Selection);

  if (m_Settings.m_IsDirty && !m_CurrentAction) SaveSettings();

  m_IsFirstFrame = false;
}

bool ed::EditorContext::DoLink(LinkId id, PinId startPinId, PinId endPinId,
...
ImVec2 ed::EditorContext::GetNodePosition(NodeId nodeId) {
  auto node = FindNode(nodeId);
  if (!node) return ImVec2(FLT_MAX, FLT_MAX);
...
void ax::NodeEditor::Detail::EditorContext::SetGroupSize(NodeId nodeId,
                                                         const ImVec2& size) {
  auto node = FindNode(nodeId);
  if (!node) {
    node = CreateNode(nodeId);
  ...
  if (IsGroup(node)) {
    node->m_GroupBounds.Max.x = node->m_GroupBounds.Min.x + size.x;
    node->m_GroupBounds.Max.y = node->m_GroupBounds.Min.y + size.y;
...
void ed::EditorContext::RestoreNodeState(Node* node) {
  ...
  if (!NodeSettings::Parse(m_Config.LoadNode(node->m_ID), *settings)) return;
...
void ed::EditorContext::ClearSelection() { m_SelectedObjects.clear(); }
...
void ed::EditorContext::DeselectObject(Object* object) {
  ...
  if (objectIt != m_SelectedObjects.end()) m_SelectedObjects.erase(objectIt);
}
...
ed::Node* ed::EditorContext::FindNodeAt(const ImVec2& p) {
  for (auto node : m_Nodes)
    if (node->TestHit(p)) return node;

  return nullptr;
}

void ed::EditorContext::FindNodesInRect(const ImRect& r, vector<Node*>& result,
                                        bool append, bool includeIntersecting) {
  if (!append) result.resize(0);

  if (ImRect_IsEmpty(r)) return;

  for (auto node : m_Nodes)
    if (node->TestHit(r, includeIntersecting)) result.push_back(node);
}

void ed::EditorContext::FindLinksInRect(const ImRect& r, vector<Link*>& result,
                                        bool append) {
  if (!append) result.resize(0);
  ...
  for (auto link : m_Links)
    if (link->TestHit(r)) result.push_back(link);
}

void ed::EditorContext::FindLinksForNode(NodeId nodeId, vector<Link*>& result,
                                         bool add) {
  if (!add) result.clear();

  for (auto link : m_Links) {
    if (!link->m_IsLive) continue;
...
bool ed::EditorContext::IsSuspended() { return m_Canvas.IsSuspended(); }

bool ed::EditorContext::IsActive() { return m_IsWindowActive; }
...
ed::Node* ed::EditorContext::CreateNode(NodeId id) {
  ...
  if (!settings) settings = m_Settings.AddNode(id);
...
static inline auto FindItemInLinear(C& container, Id id) {
  ...
  for (auto it = start; it < end; ++it)
    if ((*it).m_ID == id) return it->m_Object;
#else
  for (auto item : container)
    if (item.m_ID == id) return item.m_Object;
#endif
...
ed::Pin* ed::EditorContext::FindPin(PinId id) { return FindItemIn(m_Pins, id); }
...
void ed::EditorContext::SaveSettings() {
  ...
  for (auto& node : m_Nodes) {
    ...
    if (IsGroup(node)) settings->m_GroupSize = node->m_GroupBounds.GetSize();

    if (!node->m_RestoreState && settings->m_IsDirty &&
        m_Config.SaveNodeSettings) {
      if (m_Config.SaveNode(node->m_ID, settings->Serialize().dump(),
...
ed::Link* ed::EditorContext::FindLinkAt(const ImVec2& p) {
  for (auto& link : m_Links)
    if (link->TestHit(p, c_LinkSelectThickness)) return link;
...
void ed::EditorContext::UnregisterAnimation(Animation* animation) {
  ...
  if (it != m_LiveAnimations.end()) m_LiveAnimations.erase(it);
}
...
void ed::EditorContext::UpdateAnimations() {
  ...
  for (auto animation : m_LastLiveAnimations) {
    const bool isLive =
        (std::find(m_LiveAnimations.begin(), m_LiveAnimations.end(),
                   animation) != m_LiveAnimations.end());

    if (isLive) animation->Update();
  }
}
...
bool ed::EditorContext::AreShortcutsEnabled() { return m_ShortcutsEnabled; }

ed::Control ed::EditorContext::BuildControl(bool allowOffscreen) {
  if (!allowOffscreen &&
      !ImGui::IsWindowHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem))
    return Control(nullptr, nullptr, nullptr, nullptr, false, false, false,
                   false);
  ...
  auto checkInteractionsInArea =
      [&emitInteractiveArea, &hotObject, &activeObject, &clickedObject,
       &doubleClickedObject](ObjectId id, const ImRect& rect, Object* object) {
        if (emitInteractiveArea(id, rect)) clickedObject = object;
        if (!doubleClickedObject && ImGui::IsMouseDoubleClicked(0) &&
            ImGui::IsItemHovered())
        ...
        if (ImGui::IsItemActive()) activeObject = object;
      };
  ...
  for (auto nodeIt = m_Nodes.rbegin(), nodeItEnd = m_Nodes.rend();
    ...
    if (node->m_Type == NodeType::Group) {
      ...
      for (auto region : c_Regions) {
        ...
        if (ImRect_IsEmpty(bounds)) continue;
  ...
  if (nullptr == hotObject) hotObject = FindLinkAt(mousePos);

  // Check for interaction with background.
  auto backgroundClicked = emitInteractiveArea(NodeId(0), editorRect);
  auto backgroundDoubleClicked = !doubleClickedObject && ImGui::IsItemHovered()
  ...
  if (backgroundDoubleClicked) backgroundClicked = false;

  if (isMouseOffscreen) ImGui::PopClipRect();
...
void ed::EditorContext::ShowMetrics(const Control& control) {
  ...
  auto liveNodeCount =
      (int)std::count_if(m_Nodes.begin(), m_Nodes.end(),
                         [](Node* node) { return node->m_IsLive; });
  auto livePinCount = (int)std::count_if(
      m_Pins.begin(), m_Pins.end(), [](Pin* pin) { return pin->m_IsLive; });
  auto liveLinkCount =
      (int)std::count_if(m_Links.begin(), m_Links.end(),
                         [](Link* link) { return link->m_IsLive; });
  ...
  ImGui::BeginGroup();
  ImGui::Text("Is Editor Active: %s",
              ImGui::IsWindowHovered() ? "true" : "false");
  ImGui::Text("View Position: { x=%g y=%g }", viewRect.Min.x, viewRect.Min.y);
  ImGui::Text("View Size: { w=%g h=%g }", viewRect.GetWidth(),
              viewRect.GetHeight());
  ImGui::Text("Canvas Size: { w=%g h=%g }", canvasRect.GetWidth(),
              canvasRect.GetHeight());
  ImGui::Text("Mouse: { x=%.0f y=%.0f } global: { x=%g y=%g }", localMousePos.x,
  ...
  if (auto node = control.HotObject ? control.HotObject->AsNode() : nullptr) {
    ImGui::SameLine();
    ImGui::Text("{ x=%g y=%g w=%g h=%g }", node->m_Bounds.Min.x,
                node->m_Bounds.Min.y, node->m_Bounds.GetWidth(),
  ...
  ImGui::Text("Action Is Dragging: %s",
...
bool ed::NodeSettings::Parse(const std::string& string,
                             NodeSettings& settings) {
  auto settingsValue = json::value::parse(string);
  if (settingsValue.is_discarded()) return false;
...
bool ed::NodeSettings::Parse(const json::value& data, NodeSettings& result) {
  ...
  if (!tryParseVector(data["location"], result.m_Location)) return false;
...
ed::NodeSettings* ed::Settings::FindNode(NodeId id) {
  for (auto& settings : m_Nodes)
    if (settings.m_ID == id) return &settings;
...
void ed::Settings::ClearDirty(Node* node) {
  if (node) {
    ...
    for (auto& knownNode : m_Nodes) knownNode.ClearDirty();
  }
}
...
std::string ed::Settings::Serialize() {
  ...
  for (auto& node : m_Nodes) {
    if (node.m_WasUsed) nodes[serializeObjectId(node.m_ID)] = node.Serialize();
  }

  auto& selection = result["selection"];
  for (auto& id : m_Selection) selection.push_back(serializeObjectId(id));
...
bool ed::Settings::Parse(const std::string& string, Settings& settings) {
  ...
  if (!settingsValue.is_object()) return false;
  ...
  if (nodesValue.is_object()) {
    for (auto& node : nodesValue.get<json::object>()) {
      ...
      if (!nodeSettings) nodeSettings = result.AddNode(id);
...
ed::Animation::Animation(EditorContext* editor)
    : Editor(editor), m_State(Stopped), m_Time(0.0f), m_Duration(0.0f) {}

ed::Animation::~Animation() { Stop(); }

void ed::Animation::Play(float duration) {
  if (IsPlaying()) Stop();

  m_State = Playing;
  if (duration < 0) duration = 0.0f;
...
//
//------------------------------------------------------------------------------
ed::NavigateAnimation::NavigateAnimation(EditorContext* editor,
...
void ed::FlowAnimation::Flow(ed::Link* link, float markerDistance, float speed,
                             float duration) {
  Stop();
  ...
  if (m_MarkerDistance != markerDistance) ClearPath();
...
void ed::FlowAnimation::Draw(ImDrawList* drawList) {
  if (!IsPlaying() || !IsLinkValid() || !m_Link->IsVisible()) return;

  if (!IsPathValid()) UpdatePath();
...
void ed::FlowAnimation::UpdatePath() {
  ...
  m_Path.resize(0);
  ImCubicBezierFixedStep(collectPointsCallback, curve, step, false, 0.5f,
                         0.001f);
}
...
void ed::FlowAnimation::OnStop() { Controller->Release(this); }
...
ed::FlowAnimationController::~FlowAnimationController() {
  for (auto animation : m_Animations) delete animation;
}

void ed::FlowAnimationController::Flow(Link* link) {
  if (!link || !link->m_IsLive) return;

  auto& editorStyle = GetStyle();

  auto animation = GetOrCreate(link);

  animation->Flow(link, editorStyle.FlowMarkerDistance, editorStyle.FlowSpeed,
...
void ed::FlowAnimationController::Draw(ImDrawList* drawList) {
  ...
  for (auto animation : m_Animations) animation->Draw(drawList);
}
...
ed::FlowAnimation* ed::FlowAnimationController::GetOrCreate(Link* link) {
  // Return live animation which match target link
  {
    ...
    if (animationIt != m_Animations.end()) return *animationIt;
  }
...
const int ed::NavigateAction::s_ZoomLevelCount =
    sizeof(s_ZoomLevels) / sizeof(*s_ZoomLevels);
...
ed::EditorAction::AcceptResult ed::NavigateAction::Accept(
  ...
  if (ImGui::IsWindowHovered() /*&& !ImGui::IsAnyItemActive()*/ &&
      ImGui::IsMouseDragging(c_ScrollButtonIndex, 0.0f)) {
    m_IsActive = true;
  ...
  if (ImGui::IsWindowFocused() && ImGui::IsKeyPressed(GetKeyIndexForF()) &&
    ...
    if (!Editor->GetSelectedObjects().empty()) {
      if (m_Reason != NavigationReason::Selection ||
          m_LastSelectionId != Editor->GetSelectionId() || allowZoomIn) {
        m_LastSelectionId = Editor->GetSelectionId();
        NavigateTo(Editor->GetSelectionBounds(), allowZoomIn, -1.0f,
    ...
    if (navigateToContent)
      NavigateTo(Editor->GetContentBounds(), true, -1.0f,
                 NavigationReason::Content);
  }

  // // #debug
  // if (auto drawList = ImGui::GetWindowDrawList())
  //     drawList->AddCircleFilled(io.MousePos, 4.0f, IM_COL32(255, 0, 255,
  //     255));

  if (HandleZoom(control)) return True;
...
bool ed::NavigateAction::HandleZoom(const Control& control) {
  ...
  auto newZoom =
      MatchZoom(steps, s_ZoomLevels[steps < 0 ? 0 : s_ZoomLevelCount - 1]);
...
void ed::NavigateAction::NavigateTo(const ImRect& bounds, bool zoomIn,
                                    float duration, NavigationReason reason) {
  if (ImRect_IsEmpty(bounds)) return;

  if (duration < 0.0f) duration = GetStyle().ScrollDuration;
...
void ed::NavigateAction::NavigateTo(const ImRect& target, float duration,
...
void ed::NavigateAction::StopNavigation() { m_Animation.Stop(); }

void ed::NavigateAction::FinishNavigation() { m_Animation.Finish(); }
...
bool ed::NavigateAction::MoveOverEdge() {
  ...
  if (screenRect.Contains(screenMousePos)) return false;
...
ImVec2 ed::NavigateAction::GetViewOrigin() const { return -m_Scroll; }

float ed::NavigateAction::GetViewScale() const { return m_Zoom; }
...
float ed::NavigateAction::MatchZoom(int steps, float fallbackZoom) {
  ...
  if (currentZoomIndex < 0) return fallbackZoom;

  auto currentZoom = s_ZoomLevels[currentZoomIndex];
  if (fabsf(currentZoom - m_Zoom) > 0.001f) return currentZoom;
...
int ed::NavigateAction::MatchZoomIndex(int direction) {
  ...
  if (bestDistance > 0.001f) {
    if (direction > 0) {
      ...
      if (bestIndex >= s_ZoomLevelCount) bestIndex = s_ZoomLevelCount - 1;
    } else if (direction < 0) {
      --bestIndex;
...
bool ed::SizeAction::Process(const Control& control) {
  if (m_Clean) {
    ...
    if (m_SizedNode->m_Bounds.Min != m_StartBounds.Min ||
      ...
      Editor->MakeDirty(SaveReasonFlags::Position | SaveReasonFlags::User,
                        m_SizedNode);

    if (m_SizedNode->m_Bounds.GetSize() != m_StartBounds.GetSize() ||
        m_SizedNode->m_GroupBounds.GetSize() != m_StartGroupBounds.GetSize())
      Editor->MakeDirty(SaveReasonFlags::Size | SaveReasonFlags::User,
                        m_SizedNode);
...
void ed::SizeAction::ShowMetrics() {
  ...
  ImGui::Text("%s:", GetName());
  ImGui::Text("    Active: %s", m_IsActive ? "yes" : "no");
  ImGui::Text("    Node: %s (%p)", getObjectName(m_SizedNode),
              m_SizedNode ? m_SizedNode->m_ID.AsPointer() : nullptr);
  if (m_SizedNode && m_IsActive) {
    ImGui::Text("    Bounds: { x=%g y=%g w=%g h=%g }",
                m_SizedNode->m_Bounds.Min.x, m_SizedNode->m_Bounds.Min.y,
                m_SizedNode->m_Bounds.GetWidth(),
                m_SizedNode->m_Bounds.GetHeight());
    ImGui::Text("    Group Bounds: { x=%g y=%g w=%g h=%g }",
                m_SizedNode->m_GroupBounds.Min.x,
                m_SizedNode->m_GroupBounds.Min.y,
                m_SizedNode->m_GroupBounds.GetWidth(),
                m_SizedNode->m_GroupBounds.GetHeight());
    ImGui::Text("    Start Bounds: { x=%g y=%g w=%g h=%g }",
                m_StartBounds.Min.x, m_StartBounds.Min.y,
                m_StartBounds.GetWidth(), m_StartBounds.GetHeight());
    ImGui::Text("    Start Group Bounds: { x=%g y=%g w=%g h=%g }",
                m_StartGroupBounds.Min.x, m_StartGroupBounds.Min.y,
                m_StartGroupBounds.GetWidth(), m_StartGroupBounds.GetHeight());
    ImGui::Text("    Minimum Size: { w=%g h=%g }", m_MinimumSize.x,
                m_MinimumSize.y);
...
ed::EditorAction::AcceptResult ed::DragAction::Accept(const Control& control) {
  ...
  if (control.ActiveObject && ImGui::IsMouseDragging(0)) {
    if (!control.ActiveObject->AcceptDrag()) return False;
...
bool ed::DragAction::Process(const Control& control) {
  ...
  if (control.ActiveObject == m_DraggedObject) {
    ...
    // TODO: Move this experimental alignment to closes pivot out of internals
    // to node API
    ...
    if (!ImGui::GetIO().KeyAlt) dragOffset = alignedOffset;

    for (auto object : m_Objects) object->UpdateDrag(dragOffset);
...
ed::EditorAction::AcceptResult ed::SelectAction::Accept(
    const Control& control) {
  IM_ASSERT(!m_IsActive);
  ...
  if (control.BackgroundActive && ImGui::IsMouseDragging(0, 1)) {
    ...
    if (io.KeyCtrl) m_SelectedObjectsAtStart = Editor->GetSelectedObjects();
  ...
  if (m_IsActive) m_Animation.Stop();
...
bool ed::SelectAction::Process(const Control& control) {
  ...
  if (m_CommitSelection) {
    Editor->ClearSelection();
    for (auto object : m_CandidateObjects) Editor->SelectObject(object);
  ...
  if (ImGui::IsMouseDragging(0, 0)) {
    ...
    auto topLeft = ImVec2(std::min(m_StartPoint.x, m_EndPoint.x),
                          std::min(m_StartPoint.y, m_EndPoint.y));
    auto bottomRight = ImVec2(ImMax(m_StartPoint.x, m_EndPoint.x),
                              ImMax(m_StartPoint.y, m_EndPoint.y));
    auto rect = ImRect(topLeft, bottomRight);
    if (rect.GetWidth() <= 0) rect.Max.x = rect.Min.x + 1;
    if (rect.GetHeight() <= 0) rect.Max.y = rect.Min.y + 1;
    ...
    m_CandidateObjects.insert(m_CandidateObjects.end(),
...
void ed::SelectAction::Draw(ImDrawList* drawList) {
  if (!m_IsActive && !m_Animation.IsPlaying()) return;
  ...
  const auto outlineColor =
      Editor->GetColor(m_SelectLinkMode ? StyleColor_LinkSelRectBorder
                                        : StyleColor_NodeSelRectBorder,
                       alpha);

  drawList->ChannelsSetCurrent(c_BackgroundChannel_SelectionRect);

  auto min = ImVec2(std::min(m_StartPoint.x, m_EndPoint.x),
                    std::min(m_StartPoint.y, m_EndPoint.y));
  auto max = ImVec2(ImMax(m_StartPoint.x, m_EndPoint.x),
                    ImMax(m_StartPoint.y, m_EndPoint.y));
...
ed::EditorAction::AcceptResult ed::ContextMenuAction::Accept(
  ...
  if (isPressed || isReleased || isDragging) {
    ...
    if (auto hotObejct = control.HotObject) {
      ...
      if (candidateMenu != None) contextId = hotObejct->ID();
    } else if (control.BackgroundHot)
    ...
    if (isPressed) {
      ...
      return Possible;
    } else if (isReleased && m_CandidateMenu == candidateMenu &&
               m_ContextId == contextId) {
...
bool ed::ContextMenuAction::ShowNodeContextMenu(NodeId* nodeId) {
  if (m_CurrentMenu != Node) return false;
...
bool ed::ContextMenuAction::ShowPinContextMenu(PinId* pinId) {
  if (m_CurrentMenu != Pin) return false;
...
bool ed::ContextMenuAction::ShowLinkContextMenu(LinkId* linkId) {
  if (m_CurrentMenu != Link) return false;
...
bool ed::ContextMenuAction::ShowBackgroundContextMenu() {
  if (m_CurrentMenu != Background) return false;
...
ed::EditorAction::AcceptResult ed::ShortcutAction::Accept(
    const Control& control) {
  if (!Editor->IsActive() || !Editor->AreShortcutsEnabled()) return False;
    ...
    if (candidateAction != Paste && candidateAction != CreateNode) {
      ...
      if (!selection.empty()) {
        ...
        for (auto object : m_Context) {
          ...
          if (IsGroup(node)) node->GetGroupedNodes(extra, true);
        }
        ...
        if (!extra.empty()) {
          ...
          m_Context.erase(std::unique(m_Context.begin(), m_Context.end()),
                          m_Context.end());
        }
      ...
      if (m_Context.empty()) return False;

      // Does copying only links make sense?
      // const auto hasOnlyLinks = std::all_of(Context.begin(), Context.end(),
      // [](Object* object) { return object->AsLink() != nullptr; }); if
      // (hasOnlyLinks)
      //    return False;
          ...
          m_Context.size() > 1)  // one node cannot make connection to anything
      {
        ...
        std::for_each(m_Context.begin(), m_Context.end(),
                      [&nodes](Object* object) {
                        if (auto node = object->AsNode())
        ...
        auto isNodeInContext = [&nodes](NodeId nodeId) {
          return std::binary_search(nodes.begin(), nodes.end(),
                                    ObjectWrapper<Node>{nodeId, nullptr});
        };

        // Collect links connected to nodes and drop those reaching out of
        // context
...
bool ed::ShortcutAction::Begin() {
  if (m_IsActive) m_InAction = true;
  return m_IsActive;
}

void ed::ShortcutAction::End() {
  if (m_IsActive) m_InAction = false;
}
...
ed::EditorAction::AcceptResult ed::CreateItemAction::Accept(
  ...
  if (m_IsActive) return EditorAction::False;
...
bool ed::CreateItemAction::Process(const Control& control) {
  ...
  if (m_DraggedPin && control.ActivePin == m_DraggedPin &&
    ...
    if (control.HotPin) {
      ...
      if (m_UserAction == UserAccept) freePin = control.HotPin;
    } else if (control.BackgroundHot)
      DropNode();
    else
      DropNothing();
...
bool ed::CreateItemAction::Begin() {
  ...
  if (m_CurrentStage == None) return false;
...
ed::CreateItemAction::Result ed::CreateItemAction::QueryLink(PinId* startId,
                                                             PinId* endId) {
  IM_ASSERT(m_InActive);
  ...
  *endId = linkEndId;
  ...
  if (!m_IsInGlobalSpace) {
    ...
    ImGui::PushClipRect(rect.Min + ImVec2(1, 1), rect.Max - ImVec2(1, 1),
                        false);
...
ed::EditorAction::AcceptResult ed::DeleteItemsAction::Accept(
  ...
  auto addDeadLinks = [this]() {
    ...
    if (!links.empty()) {
      std::sort(links.begin(), links.end());
      links.erase(std::unique(links.begin(), links.end()), links.end());
      m_CandidateObjects.insert(m_CandidateObjects.end(), links.begin(),
                                links.end());
    }
  };
...
bool ed::DeleteItemsAction::Add(Object* object) {
  if (Editor->GetCurrentAction() != nullptr) return false;
...
bool ed::DeleteItemsAction::QueryLink(LinkId* linkId, PinId* startId,
                                      PinId* endId) {
  ObjectId objectId;
  if (!QueryItem(&objectId, Link)) return false;
  ...
  if (startId || endId) {
    ...
    if (startId) *startId = link->m_StartPin->m_ID;
    if (endId) *endId = link->m_EndPin->m_ID;
  }
...
bool ed::DeleteItemsAction::QueryNode(NodeId* nodeId) {
  ObjectId objectId;
  if (!QueryItem(&objectId, Node)) return false;
...
bool ed::DeleteItemsAction::QueryItem(ObjectId* itemId, IteratorType itemType) {
  if (!m_InInteraction) return false;
  ...
  if (m_CandidateItemIndex == itemCount) m_CurrentItemType = Unknown;
...
void ed::DeleteItemsAction::RemoveItem() {
  ...
  if (m_CurrentItemType == Link) Editor->NotifyLinkDeleted(item->AsLink());
}
...
ed::NodeBuilder::NodeBuilder(EditorContext* editor)
    : Editor(editor), m_CurrentNode(nullptr), m_CurrentPin(nullptr) {}
...
void ed::NodeBuilder::Begin(NodeId nodeId) {
  ...
  if (auto drawList = ImGui::GetWindowDrawList()) {
    ...
    ImDrawList_ChannelsGrow(drawList,
                            drawList->_Splitter._Count + c_ChannelsPerNode);
    drawList->ChannelsSetCurrent(m_CurrentNode->m_Channel +
                                 c_NodeContentChannel);
...
void ed::NodeBuilder::End() {
  ...
  if (auto drawList = ImGui::GetWindowDrawList()) {
    IM_ASSERT(drawList->_Splitter._Count ==
...
void ed::NodeBuilder::EndPin() {
  ...
  if (m_ResolvePivot) {
    ...
    if (m_PivotSize.x < 0) m_PivotSize.x = pinRect.GetWidth();
    if (m_PivotSize.y < 0) m_PivotSize.y = pinRect.GetHeight();
  ...
  // ImGui::GetWindowDrawList()->AddRect(m_CurrentPin->m_Bounds.Min,
  // m_CurrentPin->m_Bounds.Max, IM_COL32(255, 255, 0, 255));

  // #debug: Draw pin pivot rectangle
  // ImGui::GetWindowDrawList()->AddRect(m_CurrentPin->m_Pivot.Min,
...
void ed::NodeBuilder::Group(ImVec2& size) {
  ...
  if (IsGroup(m_CurrentNode)) size = m_CurrentNode->m_GroupBounds.GetSize();
...
ed::HintBuilder::HintBuilder(EditorContext* editor)
    : Editor(editor), m_IsActive(false), m_CurrentNode(nullptr) {}
...
bool ed::HintBuilder::Begin(NodeId nodeId) {
  ...
  if (view.Scale > 0.75f) return false;
  ...
  const auto alpha = ImMax(0.0f, std::min(1.0f, (view.Scale - c_min_zoom) /
                                                    (c_max_zoom - c_min_zoom)));
...
ed::Config::Config(const ax::NodeEditor::Config* config) {
  if (config) *static_cast<ax::NodeEditor::Config*>(this) = *config;
}
...
void ed::Config::BeginSave() {
  if (BeginSaveSession) BeginSaveSession(UserPointer);
}
...
bool ed::Config::SaveNode(NodeId nodeId, const std::string& data,
                          SaveReasonFlags flags) {
  if (SaveNodeSettings)
    return SaveNodeSettings(nodeId, data.c_str(), data.size(), flags,
                            UserPointer);
...
void ed::Config::EndSave() {
  if (EndSaveSession) EndSaveSession(UserPointer);
}
