...
struct Object {
  enum DrawFlags { None = 0, Hovered = 1, Selected = 2 };

  inline friend DrawFlags operator|(DrawFlags lhs, DrawFlags rhs) {
    return static_cast<DrawFlags>(static_cast<int>(lhs) |
                                  static_cast<int>(rhs));
  }
  inline friend DrawFlags operator&(DrawFlags lhs, DrawFlags rhs) {
    return static_cast<DrawFlags>(static_cast<int>(lhs) &
  ...
  virtual bool TestHit(const ImVec2& point, float extraThickness = 0.0f) const {
    ...
    if (extraThickness > 0) bounds.Expand(extraThickness);
  ...
  virtual bool TestHit(const ImRect& rect, bool allowIntersect = true) const {
    ...
    return !ImRect_IsEmpty(bounds) &&
           (allowIntersect ? bounds.Overlaps(rect) : rect.Contains(bounds));
  }
...
struct Pin final : Object {
  ...
  virtual void Draw(ImDrawList* drawList,
                    DrawFlags flags = None) override final;
...
inline NodeRegion operator|(NodeRegion lhs, NodeRegion rhs) {
  return static_cast<NodeRegion>(static_cast<uint8_t>(lhs) |
...
struct Link final : Object {
  ...
  virtual void Draw(ImDrawList* drawList,
                    DrawFlags flags = None) override final;
  void Draw(ImDrawList* drawList, ImU32 color,
  ...
  virtual bool TestHit(const ImVec2& point,
                       float extraThickness = 0.0f) const override final;
  virtual bool TestHit(const ImRect& rect,
                       bool allowIntersect = true) const override final;
...
struct Control {
  ...
  Control(Object* hotObject, Object* activeObject, Object* clickedObject,
          Object* doubleClickedObject, bool backgroundHot,
          bool backgroundActive, bool backgroundClicked,
          bool backgroundDoubleClicked)
      : HotObject(hotObject),
    ...
    if (hotObject) {
      ...
      if (HotPin) HotNode = HotPin->m_Node;
    }
...
struct AnimationController {
  ...
  virtual void Draw(ImDrawList* drawList) { IM_UNUSED(drawList); }
};
...
struct NavigateAction final : EditorAction {
  ...
  void NavigateTo(const ImRect& bounds, bool zoomIn, float duration = -1.0f,
...
struct DragAction final : EditorAction {
  ...
  virtual ImGuiMouseCursor GetCursor() override final {
...
struct DeleteItemsAction final : EditorAction {
  ...
  bool QueryLink(LinkId* linkId, PinId* startId = nullptr,
                 PinId* endId = nullptr);
...
struct EditorContext {
  ...
  bool DoLink(LinkId id, PinId startPinId, PinId endPinId, ImU32 color,
              float thickness);
  ...
  void FindNodesInRect(const ImRect& r, vector<Node*>& result,
                       bool append = false, bool includeIntersecting = true);
  void FindLinksInRect(const ImRect& r, vector<Link*>& result,
                       bool append = false);
  ...
  ImRect GetBounds(const std::vector<T*>& objects) {
    ...
    for (auto object : objects)
      if (object->m_IsLive) bounds.Add(object->GetBounds());

    if (ImRect_IsEmpty(bounds)) bounds = ImRect();
  ...
  ImRect GetBounds(const std::vector<ObjectWrapper<T>>& objects) {
    ...
    for (auto object : objects)
      if (object.m_Object->m_IsLive) bounds.Add(object.m_Object->GetBounds());
  ...
  void NavigateTo(const ImRect& bounds, bool zoomIn = false,
                  float duration = -1) {
    m_NavigateAction.NavigateTo(bounds, zoomIn, duration);
  }
