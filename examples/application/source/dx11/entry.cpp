...
static void CreateRenderTarget() {
  ...
  g_pd3dDevice->CreateRenderTargetView(pBackBuffer, &render_target_view_desc,
...
LRESULT WINAPI ImGui_WinProc(HWND hWnd, UINT msg, WPARAM wParam,
                             LPARAM lParam) {
...
static ImFont* ImGui_LoadFont(ImFontAtlas& atlas, const char* name, float size,
                              const ImVec2& displayOffset = ImVec2(0, 0)) {
  char* windir = nullptr;
  ...
  auto font = atlas.AddFontFromFileTTF(path.c_str(), size, &config,
                                       atlas.GetGlyphRangesChineseFull());
  if (font) font->DisplayOffset = displayOffset;
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance,
  ...
  const auto wc =
      WNDCLASSEX{sizeof(WNDCLASSEX),
                 ...
                 LoadIcon(GetModuleHandle(nullptr), IDI_APPLICATION)};
  RegisterClassEx(&wc);
  AX_SCOPE_EXIT { UnregisterClass(c_ClassName, wc.hInstance); };

  auto hwnd = CreateWindow(c_ClassName, c_WindowName.c_str(),
                           WS_OVERLAPPEDWINDOW, 1920 + 100, 100, 1440, 800,
                           nullptr, nullptr, wc.hInstance, nullptr);

  // Initialize Direct3D
  AX_SCOPE_EXIT { CleanupDeviceD3D(); };
  if (CreateDeviceD3D(hwnd) < 0) return 1;

  ImFontAtlas fontAtlas;
  auto defaultFont =
      ImGui_LoadFont(fontAtlas, "deng.ttf", 18.0f);  // 16.0f * 96.0f / 72.0f);
  fontAtlas.Build();
  ...
  ImVec4 backgroundColor =
      ImColor(32, 32, 32, 255);  // style.Colors[ImGuiCol_TitleBg];
